    ...
    <>
        ...
        <div className='article-container'>
            <div className='article-col'>
                ...
                <img
                    src='/img/atomic-ai/Overview.jpg'
                    alt=''/>
                <p>
                ...
                <p>
                    I modified an open-source C project (<a href='http://www.tckerrigan.com/Chess/TSCP/'>Tom Kerrigan's Simple Chess program</a>) using C++ and the ONNX runtime to replace the evaluation function with that from our trained model. The model itself used a multichannel bitmap representation of the chess board as input, based on work from the following paper. https://arxiv.org/abs/1908.06660
                </p>
                <img
                    src='/img/atomic-ai/atomic-ai-search.png'
                    alt=''/>
                <p>
                    We collected 900,000 games from the <a href='https://database.lichess.org/#variant_games'>lichess variant database</a>. We transform each board so that the current player is at the bottom (white). Since chess is a zero-sum game, flipping the board should just yield the negative of the evaluation in the non-flipped board. The chess pieces of both players are then transformed to a 17-channel binary-map of size 8x8 using the python-chess library which encodes the position of all pieces.
                </p>
                <p>
                    ...
                    <ul>
                        <li><span>Material value (piece value count)</span></li>
                        <li><span>Mobility score (number of legal moves available)</span></li>
                        <li><span>King safety (Closeness of friendly pieces to enemy king, and vice-versa)</span></li>
                    </ul> 
                    
                    Our results are significantly better than the ones obtained with the baseline model.
                </p>
                <img
                    src='/img/atomic-ai/modelres.PNG'
                    alt=''/>

                <p>Overall, our model is performing fairly well. It has some strong points and some limitations. We have also identified many improvements which could be made to improve the AI, some of which are directly related to deep learning, and some which are not.</p>

                <h2>Takeaways</h2>
                <p>One of the main things that we learned from our results is that the implementation of the search engine surrounding the evaluation function network is highly important to the performance of a chess AI. This means that to develop a fast AI that performs at a very high level, we need to spend considerable time optimizing search, and ensuring that our model can perform inference in a very short amount of time.</p>

                <h3>Memoryless model</h3>
                <p>Being a straightforwards CNN, our model has no ability to gain insights from sequences of board states. While this is not generally a problem, there exist a few edge cases where it is important to consider sequence dependency. These are the threefold repetition and mate-in-50 rules. These rules allow a game of chess to end in a draw under certain conditions, such as the same board position being reached three times in a game or a checkmate being unable to be reached in under 50 moves with one side having only their king remaining. While these conditions are rare, it is important to address them. We propose having a hard coded rule which modifies the evaluation function when these conditions are met.</p>

                <h3>Prioritizing important positions and parts of the board</h3>
                <p>When playing chess, human players are able to quickly identify the most critical parts of a chess board. An expert would spend little time thinking about board features and moves which are unlikely to produce advantageous results. This prioritization is not captured by our model. It would be interesting to explore the performance of a model which used attention, for instance, a transformer-based model, in this application. </p>
